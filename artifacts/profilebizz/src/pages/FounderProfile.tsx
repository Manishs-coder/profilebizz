    ...
    <div className="min-h-screen bg-[#f9f9f9] text-black">
      ...
      <div className="bg-white mt-14">
        ...
        <div className="max-w-3xl mx-auto px-6 md:px-10 pt-12 pb-10 text-center">
          ...
          {/* ── Language switch link — only show if target language exists ── */}
          {(lang === 'hi' || hasHindi) && (
...
function DynamicFounderPage({ slug, lang }: { slug: string; lang: 'en' | 'hi' }) {
  ...
  const ogTitle = `${founder.name} — ${founder.designation} | ProfileBizz`;
  const rawDesc: string = founder.oneLiner || founder.executiveSummary
    || `Read the in-depth profile of ${founder.name} on ProfileBizz.`;
...
export default function FounderProfile({ params, locale }: { params?: { slug?: string }; locale?: 'en' | 'hi' }) {
  ...
  // Known founders: use StaticFounderContent which has hardcoded Hindi (FOUNDERS_HI) + fetches DB photo
  if (FOUNDERS[slug]) return <StaticFounderContent slug={slug} lang={lang} />;
  // DB-only founders
